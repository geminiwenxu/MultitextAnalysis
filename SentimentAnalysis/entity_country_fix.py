...
def check_entity(text_df, entities=Config.entities):
    ...
    for entity in entities:
        ...
        print(total_count)
        if total_count != 0:
            per_entity.update({entity: {'num_pos': (pos_count / total_count), 'num_neu': (neu_count / total_count),
                                        'num_neg': (neg_count / total_count)}})
        else:
            continue
...
if __name__ == "__main__":
    ...
    save_to_disk(data=out,
                 ...
                 filename='sentiment_summary_entity_percentage.json')
